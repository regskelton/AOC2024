            ...
            init: function (filename) {
                ...
                xmlhttp.onreadystatechange = function () {
                    if (xmlhttp.status == 200 && xmlhttp.readyState == 4) {
                        ...
                        for (const from of connected.keys()) {
                            var tos = connected.get(from).sort();

                            var party = [...tos,from];


                            var parties= getAllSubsets(party);

                           // console.log( `Parties of ${party} = (${parties})`)
                         ...
                         //      console.log( `Trying ${p}`);
                                ...
                                if (allLinked(p)) {
                                    if( p.length > maxParty) {
                                        maxParty = p.length;

                                        console.log(`Big party: ${p.length} ${p.sort()}`);
                                    }
                            ...
                            for (const to of tos) {
                                for (const next of connected.get(to).sort()) {
                                    if (tos.includes(next)) {
                                        if (tos.includes(next)) {
                                            if (from[0] === 't' || to[0] === 't' || next[0] === 't') {
                                                const link = [from, to, next].sort();

                                                links.add("" + link[0] + "-" + link[1] + "-" + link[2]);
                                            }
                                        }
                                    }
                                }
                            ...
                            console.log(`${from} has ${tos.length} links: ${tos}`);
                        }
